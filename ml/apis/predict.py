...
def prepare_features(pr_data: dict) -> dict:
    ...
    author_assoc_en = assoc_map.get(pr_data.get("author_association", "NONE"), 3)
    ...
    features = {
        ...
        "change_ratio": np.log1p(additions) - np.log1p(deletions),
        "avg_changes_per_file": total_changes / max(changed_files, 1),
        "commits_per_file": commits_count / max(changed_files, 1),
